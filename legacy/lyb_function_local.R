...
DESeq2_DEG_analysis_batch <- function(
  ...
  if (is.null(tr_filter) || is.null(ctr_filter)) {
    stopifnot(!is.null(library), !is.null(batch), !is.null(tr), !is.null(ctr))
...
volcano_plot_scRNA <- function(
  ...
  #  gg %>% slice_head(n=top)
  #gene.list <- c((gg |> filter(avg_log2FC > 0) |> top_n(n=-top, wt=p_val))$Symbol,gene.list)
  #gene.list <- c((gg |> filter(avg_log2FC < 0) |> top_n(n=-top, wt=p_val))$Symbol,gene.list)
  gene.list <- c(
    (gg |> filter(avg_log2FC > 0) |> slice_head(n = top))$Symbol,
    gene.list
  )
  gene.list <- c(
    (gg |> filter(avg_log2FC < 0) |> slice_head(n = top))$Symbol,
